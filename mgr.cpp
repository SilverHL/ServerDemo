...
mgr::mgr( int epollfd, const host& srv ) : m_logic_srv( srv )
{
    ...
    addr.sin_port = htons( srv.m_conncnt );
...
int 
mgr::get_used_conn_cnt()
{
    return m_used.size();
}

conn* 
mgr::pick_conn( int cltfd )
{
    if ( m_conns.empty() )
    {
...
mgr::process( int fd, OP_TYPE OP )
{
    ...
    if ( connection->m_cltfd == fd )
    {
        ...
        case READ:
            {
                ...
                RET_CODE res = connection->read_from_clt();
                switch( res )
                {
                case OK:
                    {
        ...
        case WRITE:
            {
                RET_CODE res = connection->write_to_clt();
                switch( res )
                {
                case TRY_AGAIN:
                    {
    ...
    else if (connection->m_srvfd == fd)
    {
        ...
        switch( OP )
        {
            case READ:
                {
                    RET_CODE res = connection->read_from_srv();
                    switch( res )
                    {
                    case OK:
                        {
                            log( LOG_DEBUG, __FILE__, __LINE__, "conectent read from server:%s", connection->m_srv_buf);
                        }
                    case BUFFER_FULL:
                        {
            ...
            case WRITE:
                {
                    RET_CODE res = connection->write_to_srv();
                    switch( res )
                    {
                    case TRY_AGAIN:
                        {
