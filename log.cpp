...
void log ( int log_level, const char* file_name, int line_num, const char* format, ... )
{
    ...
    va_list arg_list;       //这个变量指向参数的指针
    va_start( arg_list, format );   //用va_start宏初始化变量 这个红的第二个参数是第一个可变参数的前一个参数 是一个固定参数
    memset( arg_buffer, '\0', LOG_BUFFER_SIZE );
    vsnprintf( arg_buffer, LOG_BUFFER_SIZE-1, format, arg_list );   //这个方法是用来按照特定格式打印参数
    printf( "%s\n", arg_buffer);
    fflush( stdout );
    va_end( arg_list );
}
